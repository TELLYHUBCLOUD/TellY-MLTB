...
class Merge(TaskListener):
    ...
    async def new_event(self):
        ...
        if len(self.inputs) == 1 and not is_bulk:
             ...
             if len(session["inputs"]) >= 10:
                 await send_message(self.message, f"Merge Limit Reached! Use /{BotCommands.MdoneCommand[0]} to start.")
                 return
    ...
    async def _proceed_to_download(self):
        ...
        for index, link in enumerate(self.inputs):
            ...
            if hasattr(link, "download") or isinstance(link, (str, bytes)) and hasattr(self.message, 'reply_to_message') and link == self.message.reply_to_message:
    ...
    async def on_download_complete(self):
        ...
        elif not self.name.lower().endswith(".mp4") and not self.name.lower().endswith(".mkv"):
            self.name += ".mp4"
...
async def merge_done(client, message):
    ...
    if user_id not in MERGE_SESSIONS:
        await send_message(message, f"No active merge session! Use /{BotCommands.MergeCommand[0]} to start one.")
        return
