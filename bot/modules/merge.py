...
class MergeSession:
    ...
    def add_file(self, user_id: int, file_input: Any) -> tuple[bool, str]:
        ...
        if estimated_size > 0:
            ...
            if new_total > self.MAX_TOTAL_SIZE:
                return (
                    False,
                    f"Total size would exceed limit ({get_readable_file_size(self.MAX_TOTAL_SIZE)})",
                )
        ...
        return (
            True,
            f"File added successfully ({len(session['inputs'])}/{self.MAX_FILES})",
        )
    ...
    def _get_input_id(self, file_input: Any) -> str:
        ...
        if hasattr(file_input, "message_id"):
    ...
    def _estimate_file_size(self, file_input: Any) -> int:
        ...
        if hasattr(file_input, "video") and file_input.video:
            return file_input.video.file_size or 0
        if isinstance(file_input, str) and (
            is_url(file_input) or is_telegram_link(file_input)
        ):
    ...
    def get_session_status(self, user_id: int) -> str:
        ...
        size_str = (
            get_readable_file_size(session["total_size"])
...
class MergeUtils:
    ...
    async def detect_media_type(file_path: str) -> str:
        ...
        try:
            ...
            if any(c.startswith("video") for c in codecs.get("video", [])):
                return "video"
            if any(c.startswith("audio") for c in codecs.get("audio", [])):
                return "audio"
            if codecs.get("subtitle"):
                return "subtitle"
    ...
    @staticmethod
    async def validate_merge_compatibility(
        ...
        if len(media_types) > 1 and "video" not in media_types:
        ...
        if len(media_types.get("video", [])) > 1:
            ...
            for video in media_types["video"][1:]:
                if not await validate_codec(first_video.path, video.path):
                    return (
                        True,
                        "Files have incompatible codecs - will use slower conversion method",
                    )
        ...
        try:
            ...
            for pattern, ptype in patterns:
                ...
                if len(matches) == len(files):
                    ...
                    has_video = any(
                        await MergeUtils.detect_media_type(f.path) == "video"
                        for f in files
                    )
                    has_ass = any(
                        "ass" in str(f.codecs) or "ssa" in str(f.codecs)
                        for f in files
                    )
...
class Merge(TaskListener):
    ...
    async def new_event(self):
        """Main entry point for merge command processing with improved validation."""
    ...
    async def _parse_all_inputs(
        self, text_lines: list[str], args: dict
    ) -> list[Any]:
    ...
    def _parse_telegram_range(self, link: str) -> list[str]:
        ...
        if match:
            ...
            if end - start > MERGE_SESSIONS.MAX_FILES:
                ...
                LOGGER.warning(
                    f"Trimmed Telegram range to respect max files limit: {start}-{end}"
                )
    ...
    def _deduplicate_inputs(self, inputs: list[Any]) -> list[Any]:
        ...
        for inp in inputs:
            inp_id = self._get_input_identifier(inp)
        ...
        if len(unique_inputs) != len(inputs):
            LOGGER.info(
                f"Deduplicated inputs: {len(inputs)} -> {len(unique_inputs)}"
            )
    ...
    async def _handle_session_start(self):
        ...
        if session:
            ...
            MERGE_SESSIONS.create_session(user_id, self.message, self.client)
            await send_message(
                self.message,
    ...
    async def _handle_single_file_session(self):
        ...
        if not session:
            ...
            session = MERGE_SESSIONS.create_session(
                user_id, self.message, self.client
            )
    ...
    async def _process_merge_request(self):
        ...
        try:
            ...
            can_merge, reason = await MergeUtils.validate_merge_compatibility(
                self.files
            )
            if not can_merge:
            ...
            LOGGER.exception(f"Merge processing failed: {e!s}")
    ...
    async def _download_single_file(
        self, inp: Any, directory: str, index: int
    ) -> str | None:
        ...
        try:
            if (
                hasattr(inp, "document")
                or hasattr(inp, "video")
                or hasattr(inp, "audio")
            ):
                # Telegram media
                media = inp.document or inp.video or inp.audio
                self.name = (
                    f"[{index + 1}/{self.total_batch_files}] {media.file_name}"
                )
                await TelegramDownloadHelper(self).add_download(
                    inp, directory, self.client
                )
            ...
            LOGGER.error(f"Download error for input {index}: {e!s}")
            raise
    ...
    async def _get_tg_message_from_link(self, link: str):
        ...
        try:
            ...
            if "c/" in link or chat_identifier.isdigit():
                chat_id = (
                    int(f"-100{chat_identifier}")
                    if not chat_identifier.startswith("-100")
                    else int(chat_identifier)
                )
            else:
            ...
            LOGGER.error(f"Error fetching Telegram message from {link}: {e!s}")
            return None
    ...
    async def _determine_merge_mode(self) -> MergeMode:
        ...
        try:
            ...
            LOGGER.warning(
                f"Error determining merge mode: {e!s}, defaulting to demux mode"
            )
            return MergeMode.CONCAT_DEMUX
    ...
    async def _execute_demux_merge(self):
        ...
        filter_complex_str = (
            "".join(filter_complex)
            + f"concat=n={len(self.files)}:v=1:a=1[outv][outa]"
        )
    ...
    async def _execute_merge(
        self, cmd: list[str], output_path: str, extra_cleanup: str | None = None
    ):
    ...
    async def on_upload_error(self, error: str):
        ...
        try:
            ...
            if await aiopath.exists(self.dir):
                for root, dirs, files in await sync_to_async(
                    walk, self.dir, topdown=False
                ):
                    for name in files:
...
async def merge(client, message):
    ...
    bot_loop.create_task(Merge(client, message, is_session_start=True).new_event())
...
async def merge_done(client, message):
    ...
    bot_loop.create_task(
        Merge(client, message, is_session_done=True, **arg_dict).new_event()
    )
...
async def merge_cancel(client, message):
    ...
    if user_id in MERGE_SESSIONS.sessions:
        MERGE_SESSIONS.cleanup_session(user_id)
        await send_message(message, "✅ Merge session successfully cancelled.")
    else:
...
async def merge_session_handler(client, message):
    ...
    if success:
        ...
        await send_message(message, f"❌ Error adding file:\n{msg}")
